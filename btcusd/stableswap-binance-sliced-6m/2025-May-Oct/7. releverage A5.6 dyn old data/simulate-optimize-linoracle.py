...
class Simulator:
    ...
    def single_run(self, fee, leverage):
        ...
        for d in self.simulation_data:
            ...
            target_ema = (d[PEG_TO] / ema0)**0.5
            if target_ema != ema:
                ema = ema + (target_ema - ema) / abs(target_ema - ema) * min(abs(target_ema - ema), ema * 1e-3 * (t - t_prev) / 60)
...
if __name__ == '__main__':
    ...
    for fee in np.logspace(log10(0.002), log10(0.1), 50):
        fees.append(fee)
