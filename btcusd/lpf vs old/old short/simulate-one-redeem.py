    ...
    def single_run(self, fee, leverage):
        ...
        for d in self.simulation_data:
            ...
            if self.log or self.verbose:
                ...
                r_value = (d['token0'] + d['token1'] * p) / V0 * amm.collateral - amm.debt
                r_growth = r_value / (p / ema0)
                ...
                if self.verbose:
                    ...
                    r_growths.append([t, r_growth])

        if losses:
            self.losses = losses
            self.r_growths = r_growths
...
if __name__ == '__main__':
    ...
    r_growths = np.array(simulator.r_growths[::100])
    t = [datetime.fromtimestamp(_t) for _t in losses[:, 0]]
    loss = losses[:, 1] * 100 - 100
    r_growths = r_growths[:, 1] * 100 - 100
    pylab.plot(t, loss, label="Fundamental value")
    pylab.plot(t, r_growths, c="gray", label="Redemption value")
    pylab.xlabel('Time')
